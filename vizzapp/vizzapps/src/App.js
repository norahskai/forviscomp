...
const App = () => {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        const response = await axios.get('http://localhost:5090/api/options');
  ...
  useEffect(() => {
    const fetchMaterials = async () => {
      if (selectedYear) {
        try {
          const response = await axios.get('http://localhost:5090/api/materials', {
  ...
  useEffect(() => {
    const fetchMonths = async () => {
      if (selectedYear && selectedMaterial) {
        try {
          const response = await axios.get('http://localhost:5090/api/months', {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('http://localhost:5090/api/data', {
        params: {
          year: selectedYear,
  ...
  const compareData = async () => {
    if (selectedMonth1 && selectedMonth2 && selectedYear) {
      try {
        const response = await axios.get('http://localhost:5090/api/compare', {
          params: {
            year: selectedYear,
